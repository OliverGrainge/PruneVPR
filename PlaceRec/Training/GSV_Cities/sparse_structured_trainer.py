...
def prune_linear_layer_by_l2(in_features: int, layer: nn.Linear, epoch, args):
    ...
    if in_features < layer.in_features:
        l2_norms_in = torch.norm(layer.weight.data, p=1, dim=0)
    ...
    l2_norms_out = torch.norm(layer.weight.data, p=1, dim=1)
...
def prune_conv_layer_by_l1(in_channels: int, layer: nn.Conv2d, epoch, args):
    ...
    l2_norms_out = torch.norm(
        layer.weight.data.view(layer.out_channels, -1), dim=1, p=1
    )
    ...
    if "mixvpr" in args.method.lower():
        ...
        class ConvAPPruner:
            ...
            def step(self):
                ...
                prune_conv_layer_by_l1(in_channels, layer, self.epoch, self.args)
                self.epoch += 1
